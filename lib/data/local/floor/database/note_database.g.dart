    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Note` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `title` TEXT NOT NULL, `description` TEXT NOT NULL, `createdAt` INTEGER NOT NULL)');
...
class _$NoteDao extends NoteDao {
  ...
  Stream<List<Note>> getNotesSortedByCreatedTimeAscending() {
    return _queryAdapter.queryListStream(
        'SELECT * FROM note ORDER BY createdAt ASC',
        mapper: (Map<String, Object?> row) => Note(
            id: row['id'] as int?,
            title: row['title'] as String,
            description: row['description'] as String,
